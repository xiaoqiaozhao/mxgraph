...
EditorUi.prototype.toggleFormatPanel = function(forceHide)
{
	if (this.format != null)
	{
		...
		this.formatWidth = (forceHide || this.formatWidth > 0) ? 0 : 340;
...
EditorUi.prototype.refresh = function(sizeDidChange)
{
	...
	this.sidebarContainer.style.width =(effHsplitPosition-160) + 'px';
	this.formatContainer.style.top = tmp + 'px';
	this.formatContainer.style.width = fw + 'px';
	this.formatContainer.style.display = (this.format != null) ? '' : 'none';
	
	this.diagramContainer.style.left = (this.hsplit.parentNode != null) ? (effHsplitPosition-160 + this.splitSize) + 'px' : '0px';
	this.diagramContainer.style.top = this.sidebarContainer.style.top;
	this.footerContainer.style.height = this.footerHeight + 'px';
	this.hsplit.style.top = this.sidebarContainer.style.top;
	this.hsplit.style.bottom = (this.footerHeight + off) + 'px';
	this.hsplit.style.left = (effHsplitPosition-160) + 'px';
...
EditorUi.prototype.saveNodeProp = function(cell)
{
	//获取当前的所有属性名称和value
	var nodeId ="nodeIndex"+cell.getId();
	var  graph = this.editor.graph;
	var  mycellEditor = this.editor.graph.cellEditor;
    var mm = this.editor.graph.getModel().getValue(cell);
    var mystate = '';
    var textValue = '';
    if (!mxUtils.isNode(mm))
    {
	...
	$("#"+nodeId).find("tr").each(function(){
		if(i>0){
            ...
            var ccc = tdArr.eq(1).html();//获取td的内容
            var name =  tdArr.eq(0).attr("name");
            var value = '';
			//判断input类型
			var type = tdArr.eq(1).find('input').attr("type");
			if(type==undefined||type=='dialog'||type=="hidden"){
                value = tdArr.eq(1).find('input').val();//input中的value
			}else if(type=='checkbox'){
                value = tdArr.eq(1).find('input').prop("checked");//checkbox中的value
			}else if(type=='radio'){
				var len = tdArr.eq(1).find('input').length;
                for(var a=0;a<len;a++){
                	var bol = tdArr.eq(1).find('input')[a].checked;
                	if(bol){
                        value = tdArr.eq(1).find('input')[a].value;
					}
				}
            }
			...
			//暂时修改
			if(name==="name"){
                // graph.labelChanged(cell, value, null);
                 mystate =  graph.view.getState(cell);
                textValue = value;
                if(cell.value!=null){
                    cell.setAttribute("label",textValue);
				}else{
                    var doc = mxUtils.createXmlDocument();
                    var obj = doc.createElement('object');
                    mm = obj;
                    mm.setAttribute('label', textValue);
				}

			}

            mm.setAttribute(name,value);
		}
       i++;
    })

    this.editor.graph.getModel().setValue(cell, mm);
