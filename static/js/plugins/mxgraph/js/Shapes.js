...
(function()
{
	...
	// OrEllipseShape1
	function OrEllipseShape1()
	{
		mxEllipse.call(this);
	};
	mxUtils.extend(OrEllipseShape1, mxEllipse);
	OrEllipseShape1.prototype.paintVertexShape = function(c, x, y, w, h)
	{
		mxEllipse.prototype.paintVertexShape.apply(this, arguments);
	...
	mxCellRenderer.registerShape('orEllipse1', OrEllipseShape1);
	...
	mxUtils.extend(mytriangle, mxRectangleShape);
	mytriangle.prototype.isHtmlAllowed = function()
	{
			return false;
	};
	mytriangle.prototype.paintForeground = function(c, x, y, w, h)
	{
			var length = Math.min(w / 5, h / 5);
			// <path d="M 1546 633 L 1546 703 M 1476 668 L 1616 668" fill="none" stroke="rgb(42,161,252)" stroke-miterlimit="10" pointer-events="all"></path>
			// <path d="M 1621 452 L 1621 570 M 1621 452 L 1700 511 M 1621 570  L 1700 511" fill="none" stroke="rgb(42,161,252)" stroke-miterlimit="10" pointer-events="all"></path>
			// console.log("c="+c+";x="+x+";y="+y+";w="+w+";h="+h);
			c.begin();
			// c.setStrokeColor("red");
			// c.setFillColor(this.stroke);

			c.moveTo(x + w / 2-length, y + h/2-length);
			c.lineTo(x + w / 2+length, y + h/2 );
			c.moveTo(x + w / 2-length, y + h/2-length);
			c.lineTo(x + w / 2-length, y + h/2+length);
			c.moveTo(x + w / 2-length, y + h/2+length);
			c.lineTo(x + w / 2+length, y + h/2 );
			c.end();
			c.stroke();
	...
	mxCellRenderer.registerShape('mytriangle', mytriangle);
	// SortShape1  
	function SortShape1()
	{
		mxRhombus.call(this);
	};
	mxUtils.extend(SortShape1, mxRhombus);
	SortShape1.prototype.paintVertexShape = function(c, x, y, w, h)
	{
		mxRhombus.prototype.paintVertexShape.apply(this, arguments);
		var border = Math.min(w / 5, h / 5) + 1;
	...
	mxCellRenderer.registerShape('sortShape1', SortShape1);
	// closeLines
	function closeLines()
	{
		mxRhombus.call(this);
	};
	mxUtils.extend(closeLines, mxRhombus);
	closeLines.prototype.paintVertexShape = function(c, x, y, w, h)
	{
		mxRhombus.prototype.paintVertexShape.apply(this, arguments);
		var s2 = 0.7;
	...
	mxCellRenderer.registerShape('closeLines', closeLines);
	// zqq加end
