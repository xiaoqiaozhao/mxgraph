...
Format.prototype.refresh = function()
{
    ...
    // var xmlValue = $("#xmlContent").val();
    var xmlValue = sessionStorage.getItem("xmlContent");

    if(xmlValue!=null&&xmlValue.length>0) {
        if (!xmlValue.startsWith("root")) {
            xmlValue = '<root>' + xmlValue + '</root>';
        }
        if (!xmlValue.startsWith("mxGraphModel")) {
            xmlValue = '<mxGraphModel dx="887" dy="474" grid="1" gridSize="10" guides="1" tooltips="1" connect="1" arrows="1" fold="1" page="1" pageScale="1" pageWidth="827" pageHeight="1169">' + xmlValue + '</mxGraphModel>';
        }
        var data = graph.zapGremlins(mxUtils.trim(xmlValue));
        ...
        // $("#xmlContent").val('');
        sessionStorage.setItem("xmlContent","");
    }
    //保存当前节点属性值
	...
	label.style.height = (mxClient.IS_QUIRKS) ? '34px' : '35px';
    ...
    preDiv.style.whiteSpace = 'nowrap';
    preDiv.style.color = 'rgb(112, 112, 112)';
    preDiv.style.textAlign = 'left';
    preDiv.style.cursor = 'default';
    this.container.appendChild(preDiv);

    var propDiv = document.createElement('div');
    propDiv.className = 'geFormatSection';
    propDiv.style.padding = '0px';
    propDiv.style.position = 'relative';

    var propTab =document.createElement('table');
    propTab.style.whiteSpace = "nowrap";
    propTab.style.width = "100%";
    var propTbaody = propTab.createTBody();
    var nodeIndexId = "nodeIndex"+cell.getId();
    propTbaody.setAttribute("id",nodeIndexId);
    ...
    // // th.innerHTML = "value";
    // hrow.appendChild(th);
    propTbaody.appendChild(hrow);

    // editorUi.prototype.showNodeProp(graph.getSelectionCell());
	//判断选择的节点类型

  // var props = canvasProperties; //画布
	// var symbol = '';
	var isCanvas =true;
	var cell = graph.getSelectionCell() || graph.getModel().getRoot();
	...
	// 	if(style == "shape=image;image=static/js/plugins/mxgraph/jscolor/start.png"){
	// 		props = startProperties;
	// 	}else if(style == "shape=image;image=static/js/plugins/mxgraph/jscolor/end.png"){
	// 		props = endProperties;
	// 	}else if(style == "shape=image;image=static/js/plugins/mxgraph/jscolor/open.png"){
	// 		props = exclusiveGwProperties;
	// 	}else if(style == "shape=image;image=static/js/plugins/mxgraph/jscolor/close.png"){
	// 		props = parallelGwProperties;
	// 	}else if(cell.value == "User Task"){
	// 		props = userTaskProperties;
	// 	}else if(cell.value == "Sub-Process"){
	// 		props = subProcessProperties;
	// 	}else{
	// 		isCanvas =false;
	// 	}
	// }
	...
	// 	var style =cell.style;
	// 	if(style!=undefined){
	// 		if(style.endsWith(";")) {//这里可以替换为startsWith
	// 			style = style.substring(0,style.length-1);
	// 		}
	// 		var ll = style.split(";").length-1;
	// 		var sym = style.split(";")[ll];
	// 		if(sym.startsWith("\"")){
  ...
  //任务
	...
	// 	if(symbol=="userTask"){
	// 		props = userTaskProperties;
	// 	}else if(symbol=="serviceTask"){
	// 		props = serviceTaskProperties;
	// 	}else if(symbol=="start"||symbol=="end"){
	// 		props = startProperties;
	// 	}else if(symbol=="exclusiveGw"){
	// 		props = exclusiveGwProperties;
	...
	modelDesc = GetQueryString('modelDesc')

    // for(var i=0;i<props.length;i++){
    // 	var prop = props[i];
    // 	if(isCanvas){
    //         var pName =prop.name;
    // 		if(pName=="processId"&&modelKey!=null&&modelKey!="null"){
    //             prop.defVal = modelKey;
		// 	}
    //         if(pName=="processName"&&modelName!=null&&modelName!="null"){
    //             prop.defVal = modelName;
    //         }
    //         if(pName=="desc"&&modelDesc!=null&&modelDesc!="null"){
    //             prop.defVal = modelDesc;
    //         }
		// }
    //     var tr =createPropertiesTr(propDiv,prop,nodeIndexId,isCanvas);
    //     propTbaody.appendChild(tr);
    // }


    // propTab.appendChild(propTbaody);
    // propDiv.appendChild(propTab);

// 新增部分zqq
    // btnSave = mxUtils.button("保存", mxUtils.bind(this, function(evt)
    // {
		// 	var modelId = GetQueryString('modelId')
		// 	var xmlContent = htworkflowXmlDialog(ui);
		// 	if(xmlContent==''){
		// 		alert("画布为空");
		// 		return false;
		// 	}
		...
		// 	}
		// 	$.ajax({
		// 		// url:"http://172.168.3.26:8888/insertOrUpdateOsReBatearray",
    //             url:basePath+"insertOrUpdateOsReBatearray",
    //     type:"post",
		...
		// 	});
		// 	// alert("保存按钮")
		// 	// this.editorUi.actions.get('saveNodeProp').funct();
				
    // }));
    // btnSave.setAttribute('title',  '保存');
    // btnSave.className += 'btnSave';
    // propDiv.appendChild(btnSave);
    // preDiv.appendChild(propDiv);


    // btnLook = mxUtils.button("查看流程定义", mxUtils.bind(this, function(evt)
    // {
    //     flowXmlDetailDialog(htworkflowXmlDialog(ui));
    // }));
    // btnLook.setAttribute('title',  '查看流程定义');
    // btnLook.className += 'btnLook';
    // propDiv.appendChild(btnLook);
    // preDiv.appendChild(propDiv);
    // 新增部分zqq  end
	if(!isCanvas) {
		btn = mxUtils.button("保存", mxUtils.bind(this, function (evt) {
			this.editorUi.actions.get('saveNodeProp').funct();
		}));

		btn.setAttribute('title', '保存');
		btn.className += 'btnCanvas';
		// btn.setAttribute('title',  '保存 (' + this.editorUi.actions.get('editData').shortcut + ')');
		btn.style.width = '100px';
		btn.style.marginBottom = '2px';
		propDiv.appendChild(btn);
	}else{
    // this.editorUi.actions.get('saveNodeProp').funct();   //报错处，待解决
	}
	preDiv.appendChild(propDiv);
   ...
   function loadNodeProp(){
    ...
    function createPropertiesTr(propDiv,prop,nodeIndexId,isCanvas){
        var that =this;
             ...
             if(attrs!=undefined){
                 for (var i = 0; i < attrs.length; i++){
                     if(attrs[i].name==pName){
                         pValue = attrs[i].value;
                     }
                 }
			 }
        }
		if(pValue==''){
            pValue =  prop.defVal; //or undefined if defVal is undefined
		}
		if(pType=='accOrcanhidden'){
			row.setAttribute("style","display:none");
		}
        row.className = 'gePropRow' +  'Dark' +'Alt'+ ' gePropNonHeaderRow';
        ...
        } else if (pType == 'nodeTypeRadio')
        {
            var input1 = document.createElement('input');
            input1.type = 'radio';
            input1.setAttribute("name",pName);
            ...
            input2.type = 'radio';
            input2.setAttribute("name",pName);
        ...
        else if (pType == 'enum')
        {
            ...
            mxEvent.addListener(td, 'click', mxUtils.bind(that, function()
            {
                ...
                setElementPos(propDiv,td, select);
        ...
        else if (pType == 'staticArr')
        {
        ...
        } else if(pType == 'dialogTextArea'){
            var input = document.createElement('input');
            input.setAttribute("name",pName);
            input.setAttribute("onclick","dialogTextArea('"+nodeIndexId+"','"+pName+"')");
            input.setAttribute("value",pValue);
            td.appendChild(input);
        }else if(pType == 'assigneecq'||pType == 'candidatescq'){//重庆用
            var input = document.createElement('input');
            input.setAttribute("name",pName);
            input.setAttribute("value",pValue);
						var button = document.createElement('button');
						button.className = 'handleConfig'
            button.setAttribute("style","width:40px;height:20px;")
						button.setAttribute("onclick","userFuncq('"+nodeIndexId+"','"+pName+"hidden')");
						
            button.innerText="配置";
            td.appendChild(input);
            td.appendChild(button);
        }else if(pType == 'accOrcanhidden'||pType == 'accOrcanhidden'){//重庆用
        ...
        }else if(pType == 'assigneeDialog'||pType == 'candidatesDialog'){
            var input = document.createElement('input');
            input.setAttribute("name",pName);
			input.setAttribute("type","hidden");
            input.setAttribute("value",pValue);
			...
			var button = document.createElement('button');
			button.setAttribute("style","width:40px;height:20px;")
			button.setAttribute("onclick","userDialog('"+nodeIndexId+"','"+pName+"')");
			button.innerText="配置";
			td.appendChild(input);
			td.appendChild(button);
        }else if(pType == 'funDialog'){
            var input = document.createElement('input');
            input.setAttribute("name",pName);
            input.setAttribute("type","hidden");
            input.setAttribute("value",pValue);
            ...
            var button = document.createElement('button');
            button.setAttribute("style","width:40px;height:20px;")
            button.setAttribute("onclick","funDialog('"+nodeIndexId+"','"+pName+"')");
            button.innerText="配置";
            td.appendChild(input);
            td.appendChild(button);
        }else if(pType == 'dialogAssignUser'){
            var input = document.createElement('input');
            input.setAttribute("name",pName);
            input.setAttribute("onclick","dialogAssignUser('"+nodeIndexId+"','"+pName+"')");
            input.setAttribute("value",pValue);
            td.appendChild(input);
        }else if(pType == 'dialogList'){
            var input = document.createElement('input');
            input.setAttribute("name",pName);
            input.setAttribute("onclick","dialogList('"+nodeIndexId+"','"+pName+"')");
            input.setAttribute("value",pValue);
            td.appendChild(input);
        }else if(pType == 'dialog'){
            var input = document.createElement('input');
            input.setAttribute("name",pName);
            input.setAttribute("onclick","dialog('"+nodeIndexId+"','"+pName+"')");
            input.setAttribute("value",pValue);
            td.appendChild(input);
        }else if(pType == 'mqDialogList'){
        	//<td class="gePropRowCell"><input name="preMqs" type="hidden" value=""><button style="width:40px;height:20px;" onclick="mqDialogList('nodeIndex2','preMqs')">配置</button></td>
            var input = document.createElement('input');
            input.setAttribute("name",pName);
            input.setAttribute("type","hidden");
            ...
            var button = document.createElement('button');
            button.setAttribute("style","width:40px;height:20px;")
            button.setAttribute("onclick","mqDialogList('"+nodeIndexId+"','"+pName+"')");
            button.innerText="配置";
            td.appendChild(input);
            td.appendChild(button);
        }else if(pType == 'editDialogList'){
            var input = document.createElement('input');
            input.setAttribute("name",pName);
            input.setAttribute("onclick","editDialogList('"+nodeIndexId+"','"+pName+"')");
            input.setAttribute("value",pValue);
            td.appendChild(input);
        }
        else if(pType == 'date'){
            var input = document.createElement('input');
            input.setAttribute("value",pValue);
            input.setAttribute("onclick","WdatePicker()");
            td.appendChild(input);
        }
        ...
        }else
        {
            ...
            {
                var input = document.createElement('input');
                setElementPos(propDiv,td, input, true);
                input.value = pValue;
    ...
    function applyStyleVal(pName, newVal, prop, delIndex)
    {
      ...
      /* graph.getModel().beginUpdate();
        try
        {
            ...
            ui.fireEvent(new mxEventObject('styleChanged', 'keys', changedProps,
    ...
    function setElementPos(propDiv,td, elem, adjustHeight)
    {

        var divPos = mxUtils.getOffset(propDiv, true);
